...
public class DeviceManagerTest {
    ...
    public void testReplyToRegistrationRequests() {
        ...
        managerActor.tell(new DeviceManager.RequestTrackDevice("group01", "device01", probe.getRef()));
        DeviceManager.DeviceRegistered registered1 = probe.receiveMessage();

        //设备组Actor(group01)发送消息监控注册（group01，device02，设备组Actor）
        managerActor.tell(new DeviceManager.RequestTrackDevice("group01", "device02", probe.getRef()));
    ...
    public void testIgnoreWrongRegistrationRequests() {
        TestProbe<DeviceManager.DeviceRegistered> probe = testKit.createTestProbe(DeviceManager.DeviceRegistered.class);
        ActorRef<DeviceManager.Command> managerActor = testKit.spawn(DeviceManager.create());
        managerActor.tell(new DeviceManager.RequestTrackDevice("wrongGroup", "device01",
                probe.getRef()));
    ...
    public void testListActiveDevices() {
        ...
        managerActor.tell(new DeviceManager.RequestTrackDevice("group01", "device01", registeredProbe.getRef()));
        //在默认超时期限内接收一个类型为“M” DeviceRegistered 的消息。
        registeredProbe.receiveMessage();

        managerActor.tell(new DeviceManager.RequestTrackDevice("group01", "device02", registeredProbe.getRef()));
        registeredProbe.receiveMessage();

        //查询活跃的Actor
        TestProbe<DeviceManager.ReplyDeviceList> deviceListProbe = testKit.createTestProbe(DeviceManager.ReplyDeviceList.class);
        managerActor.tell(new DeviceManager.RequestDeviceList(0L, "group01", deviceListProbe.getRef()));
    ...
    public void testListActiveDevicesAfterOneShutsDown() {
        ...
        managerActor.tell(new DeviceManager.RequestTrackDevice("group01", "device01",
                registeredProbe.getRef()));
        DeviceManager.DeviceRegistered registered1 = registeredProbe.receiveMessage();

        managerActor.tell(new DeviceManager.RequestTrackDevice("group01", "device02",
                registeredProbe.getRef()));
        ...
        registeredProbe.awaitAssert(
                () -> {
                    managerActor.tell(new DeviceManager.RequestDeviceList(1L, "group01", deviceListProbe.getRef()));
