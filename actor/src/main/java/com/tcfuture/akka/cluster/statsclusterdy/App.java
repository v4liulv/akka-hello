...
public class App {
    ...
    static final ServiceKey<Message.ProcessText> STATS_SERVICE_KEY = ServiceKey.create(Message.ProcessText.class, "StatsService");
        ...
        static Behavior<Void> create() {
            return Behaviors.setup(context -> {
                ...
                if (cluster.selfMember().hasRole("compute")) {
                    ...
                    //创建workers actor
                    Behavior<Message.Process> workerPoolBehavior = Routers.pool(numberOfWorkers, StatsWorker.create().<Message.Process>narrow())
                            //通过使用一致的哈希路由消息。暂时未知干嘛~~~
                            .withConsistentHashingRouting(1, process -> process.word);
                    ActorRef<Message.Process> workers = context.spawn(workerPoolBehavior, "WorkerRouter");
                    //循环创建worker actor
                   /* for (int i = 0; i < numberOfWorkers; i++) {
                        context.spawn(StatsWorker.create(), "StatsWorker" + i);
                    }*/

                    //创建Service actor 把当前创建workers actor作为参数
                    ActorRef<Message.CommandService> service = context.spawn(StatsService.create(), "StatsService");
                ...
                if (cluster.selfMember().hasRole("client")) {
                    ...
                    ActorRef<Message.ProcessText> serviceRouter = context.spawn(Routers.group(STATS_SERVICE_KEY), "ServiceRouter");
